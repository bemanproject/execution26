...
struct impls_for<split_impl_t> : ::beman::execution26::detail::default_impls {
    ...
    struct split_receiver {
        ...
        split_receiver(shared_state<Sndr>* state) noexcept : sh_state(state) {
            if (sh_state) {
                sh_state->inc_ref();
            }
        }
    ...
    struct shared_state {
        ...
        void add_listener(local_state_base* listener) noexcept {
            ...
            if (auto maybe_ptr = waiting_states.try_push(listener); maybe_ptr) {
                // the queue was not shutdown, we check if the operation is already started
                auto ptr = *maybe_ptr;
                if (!ptr) {
                    // the operation was not started yet, we start it
                    assert(op_state);
    ...
    struct local_state final : local_state_base {
        ...
        auto notify() noexcept -> void override {
            ...
            if (stop_token.stop_requested()) {
                ...
                ::std::visit(
                    [&]<class Arg>(Arg&& arg) noexcept -> void {
