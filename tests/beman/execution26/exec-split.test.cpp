...
struct some_thread_pool_scheduler {
    ...
    struct timed_sender {
        ...
        template <std::same_as<beman::execution26::set_value_t> Tag>
        auto get_completion_scheduler() const noexcept -> some_thread_pool_scheduler {
...
void test_completion_from_another_thread() {
    ...
    auto [val]     = beman::execution26::sync_wait(return_42).value();
    ASSERT(val == 42);
}
